...
class TestCmdTestNoChanges:
    ...
    def test_test_command_no_changes(self) -> None:
        ...
        with patch("diff_guard.utils.git.get_repo_root", return_value=Path("/tmp/fake")), \
...
class TestCmdTestWithChanges:
    ...
    def test_human_readable_output(self) -> None:
        ...
        with patch("diff_guard.utils.git.get_repo_root", return_value=Path("/tmp/fake")), \
            ...
            MockFG.return_value.build.return_value = None
            ret = cmd_test(args)
    ...
    def test_command_only_flag(self) -> None:
        ...
        with patch("diff_guard.utils.git.get_repo_root", return_value=Path("/tmp/fake")), \
             patch("diff_guard.core.diff_parser.get_staged_diff", return_value="not-empty"), \
             patch("diff_guard.core.diff_parser.parse_unified_diff", return_value=[
                 MagicMock(file_path="src/app.py", is_deleted=False)
             ]), \
             patch("diff_guard.utils.file_graph.FileGraph") as MockFG, \
             patch("diff_guard.core.test_mapper.TestMapper", return_value=mock_mapper):
            MockFG.return_value.build.return_value = None
...
class TestCmdCheckJsonOutput:
    ...
    def test_check_command_json_output(self, capsys: object) -> None:
        ...
        with patch("diff_guard.utils.git.get_repo_root", return_value=Path("/tmp/fake")), \
             ...
             patch("diff_guard.core.phantom_change_detector.PhantomChangeDetector", return_value=mock_phantom_detector), \
             patch("diff_guard.core.blast_radius.BlastRadiusAnalyzer", return_value=mock_blast_analyzer), \
             patch("diff_guard.core.regression_risk_scorer.RegressionRiskScorer", return_value=mock_risk_scorer), \
...
class TestCmdCheckFailOn:
    ...
    def _run_check_with_risk(self, extra_args: list[str], risk_level: str, scope_confidence: float = 0.8) -> int:
